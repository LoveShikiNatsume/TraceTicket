...
class CSVFileMonitor:
    ...
    def print_csv_result(self, csv_status: Dict[str, Any]):
        ...
        # 获取基本信息
        status = csv_status.get("status", "UNKNOWN")
        csv_file = csv_status.get("csv_file", "unknown.csv")
        total_traces = csv_status.get("total_traces", 0)
        normal_traces = csv_status.get("normal_traces", 0)
        anomaly_traces = csv_status.get("anomaly_traces", 0)
        anomaly_percentage = csv_status.get("anomaly_percentage", 0.0)
        detection_time = csv_status.get("detection_timestamp", "")
        ...
        # 顶部装饰线
        print("\n" + "═" * 80)
        print(f"{'':^20}{status_color}🔍 CSV文件异常检测结果 🔍{reset_color}{'':^20}")
        print("═" * 80)
        
        # 文件信息区域
        print(f"📁 文件名称: {status_color}{csv_file}{reset_color}")
        print("─" * 80)
        
        # 检测结果核心信息
        print(f"{status_icon} 检测结果: {status_color}{result_text}{reset_color}")
        print(f"📊 总体统计:")
        print(f"   ├─ 总 traces 数量: {total_traces:>6}")
        print(f"   ├─ 正常 traces:   {normal_traces:>6} ✅")
        print(f"   ├─ 异常 traces:   {anomaly_traces:>6} 🚨")
        print(f"   └─ 异常比例:      {anomaly_percentage:>6.2f}%")
        
        
        # 保存JSON文件
        csv_filename = csv_status.get('csv_file', 'unknown')
        if csv_filename.endswith('.csv'):
            output_file = csv_filename.replace('.csv', '.json')
        else:
        ...
        print(f"💾 详细结果已保存至: {status_color}{output_file}{reset_color}")
        print("═" * 80)
        
        
        # 创建results目录
        project_root = Path(__file__).parent  # 获取当前脚本所在目录
        results_dir = project_root / "results"
        results_dir.mkdir(exist_ok=True)  # 如果目录不存在就创建
        ...
        if csv_filename.endswith('.csv'):
            ...
            output_filename = f"{csv_filename}.json"
