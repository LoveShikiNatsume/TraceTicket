...
class Query:
    ...
    def query_high_speed_ticket(self, place_pair: tuple = (), time: str = "", headers: dict = {}) -> List[str]:
        ...
        try:
            ...
            if response.status_code != 200 or response.json().get("data") is None:
                print(f"query high speed ticket failed: status {response.status_code}")
                return []
            ...
            print(f"query high speed ticket error: {e}")
            return []
    ...
    def query_normal_ticket(self, place_pair: tuple = (), time: str = "", headers: dict = {}) -> List[str]:
        try:
            ...
            if response.status_code != 200 or response.json().get("data") is None:
                print(f"query normal ticket failed: status {response.status_code}")
                return []
            ...
            print(f"query normal ticket error: {e}")
            return []
    ...
    def query_orders(self, types: tuple = tuple([0, 1]), query_other: bool = False, headers: dict = {}) -> List[tuple]:
        ...
        try:
            ...
            response = self.session.post(url=url, headers=headers, json=payload)
            if response.status_code != 200 or response.json().get("data") is None:
                print(f"query orders failed: status {response.status_code}")
                return []
    ...
    def pay_order(self, order_id: str, trip_id: str, headers: dict = {}) -> str:
        try:
            ...
            if res.status_code == 200:
                print(f"order {order_id} pay success")
                return order_id
            else:
                print(f"pay order {order_id} failed: status {res.status_code}")
                return None
        except Exception as e:
            print(f"pay order {order_id} error: {e}")
            return None
    ...
    def cancel_order(self, order_id, headers: dict = {}):
        try:
            ...
            if res.status_code == 200:
                print(f"order {order_id} cancel success")
                return order_id
            else:
                print(f"cancel order {order_id} failed: status {res.status_code}")
                return None
        except Exception as e:
    ...
    def collect_order(self, order_id, headers: dict = {}):
        try:
            ...
            res = self.session.get(url=url, headers=headers)
            if res.status_code == 200:
                print(f"order {order_id} collect success")
                return order_id
            else:
                print(f"collect order {order_id} failed: status {res.status_code}")
                return None
        except Exception as e:
            print(f"collect order {order_id} error: {e}")
            return None
    ...
    def enter_station(self, order_id, headers: dict = {}):
        try:
            ...
            res = self.session.get(url=url, headers=headers)
            if res.status_code == 200:
                print(f"order {order_id} enter station success")
                return order_id
            else:
                print(f"enter station {order_id} failed: status {res.status_code}")
                return None
        except Exception as e:
            print(f"enter station {order_id} error: {e}")
            return None
    ...
    def preserve(self, start: str, end: str, trip_ids: List = [], is_high_speed: bool = True, date: str = "", headers: dict = {}):
        try:
            ...
            if not trip_ids:
                print("no trip ids available for preserve")
                return
            ...
            if res.status_code == 200 and res.json()["data"] == "Success":
                print(f"preserve trip {trip_id} success")
            else:
                print(f"preserve trip {trip_id} failed: status {res.status_code}")
        except Exception as e:
