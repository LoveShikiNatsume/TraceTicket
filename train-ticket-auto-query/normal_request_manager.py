...
def main_thread(duration_minutes=60):
    threads = []

    start_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    print(f"start:{start_time}, will run for {duration_minutes} minutes")

    for i in range(5):
        t = Thread(name="thread" + str(i), target=main, args=(duration_minutes,))
        time.sleep(1)
        t.start()
        threads.append(t)
